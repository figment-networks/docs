...
[Juno](https://www.junonetwork.io/) is a decentralized, public, permissionless network for cross-chain smart contracts. It comes with the CosmWasm module, which enables users to run smart contracts on a WebAssembly (WASM) virtual machine. By utilizing the Inter Blockchain Communication (IBC) protocol, Juno is highly interoperable with other Cosmos chains. You can visit the [official documentation](https://docs.junonetwork.io/juno/readme) for more information.

#### Networks

Juno mainnet is `juno-1`. Juno current testnet is `uni-3`.
